...
- **Фронтенд** — статический PWA, чистый HTML/CSS/JS без сборки (`index.html`,
  ...
  хранилище — `localStorage`, синк идёт фоном.
- **Бэкенд** — один Go-бинарь (`backend/`): раздаёт само приложение и API синка
  с одного origin, TLS через Let's Encrypt (`autocert`). Разворачивается на VM
